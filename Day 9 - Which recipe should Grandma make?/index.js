...
function filterRecipes(guestName, availableRecipes) {
  const suitableRecipes = availableRecipes.filter((recipe) => {
    const likedIngredients = recipe.ingredients.some(ingredient => 
    guestName.loves.includes(ingredient));
    const dislikedIngredients = recipe.ingredients.some(ingredient =>
    guestName.dislikes.includes(ingredient));
    return likedIngredients && !dislikedIngredients;
  });
    console.log(`List of ${guestName.name}' suitable recipes:`);
    suitableRecipes.forEach((recipe) => {
      console.log(`${recipe.name} -> Ingredients: ${recipe.ingredients}`);
  });
}

// Step 2: Output the suitable recipes
filterRecipes(guest, recipes);