...
function displayEmoji() {
    ...
    if (currentMovie) {
        emojiCluesContainer.innerText = currentMovie.emoji;
...
function handleGuess(event) {
    ...
    if (guess.toLowerCase() === currentMovie.title.toLowerCase()) {
        ...
        if (remainingGuesses > 0) {
            messageContainer.innerText = `Incorrect! You have ${remainingGuesses} more guesses remaining.`;
        } else {
            messageContainer.innerText = `The film was ${currentMovie.title}!`;
            setTimeout(() => {
                displayEmoji();
            }, 3000);
        }
